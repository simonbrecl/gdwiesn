...
public class EndLevel extends World {
	...
	public EndLevel(int day, int rentGoal, int moneyCount, TentState state) {
		// Create a new world with 600x400 cells with a cell size of 1x1 pixels.
		super(800, 600, 1);

		System.out.println("Money made: " + moneyCount);
		this.tentState = state;
		money = state.getMoney() + moneyCount - rentGoal;
		...
		if (day > 1) {
			savings = new GreenfootImage(state.getMoney() + "", 26, Color.BLACK, new Color(0, 0, 0, 0));
		...
		goal = new GreenfootImage(rentGoal + "", 26, Color.BLACK, new Color(0, 0, 0, 0));
		getBackground().drawImage(goal, 403 + offset, 419);

		if (moneyCount > rentGoal) {
		...
		rent = new GreenfootImage(rentGoal + "", 26, Color.BLACK, new Color(0, 0, 0, 0));
